...
template <typename T>
void graph_hash_of_mixed_weighted_binary_operations_erase_infty(std::vector<std::pair<int, T>>& input_vector, int key) {
	...
	if (input_vector.size() > 0) {
		...
		while (left <= right) {
			int mid = left + ((right - left) / 2);
			if (input_vector[mid].first == key && input_vector[mid].second == std::numeric_limits<T>::max()) {
				input_vector.erase(input_vector.begin() + mid);
				break;
			}
			else if (input_vector[mid].first > key) {
