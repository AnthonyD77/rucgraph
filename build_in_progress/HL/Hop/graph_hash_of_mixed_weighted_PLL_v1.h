...
void update_2019R1_condition_PLL_with_non_adj_reduction(int v1, int ideal_graph_size, vector<int> *reduction_measures_2, vector<int> *f_2019R1)
{
    ...
    for (int v2 = v1 + 1; v2 < ideal_graph_size; v2++)
    {
        ...
        if (ideal_graph_599[v1].size() > ideal_graph_599[v2].size())
            break; // no need to j++ any more

        int condition;

        if (graph_v_of_v_idealID_contain_edge(ideal_graph_599, v1, v2))
        { // may be equivalent_2
            bool is_equivalent_2 = true;
            int size = ideal_graph_599[v1].size();
            auto it1 = ideal_graph_599[v1].begin();
            auto it2 = ideal_graph_599[v2].begin();
            while (it1 != ideal_graph_599[v1].end() && it2 != ideal_graph_599[v2].end())
            {
                if (it1->first == v2)
                {
                    it1++;
        ...
        { // may be equivalent_1
            ...
            auto it1 = ideal_graph_599[v1].begin();
            auto it2 = ideal_graph_599[v2].begin();
            while (it1 != ideal_graph_599[v1].end())
            {
                if (it1->first == it2->first)
                {
                    if (abs(it1->second - it2->second) > 1e-5)
        ...
        /*there are bugs below; 1. there should be locks below 2. even with locks, parallelly updating f function could
        cause wrong f mapping; but it seems that such bugs never occur, since v1 is increasing pushing into threads*/
        if (condition == 1)
        {
            (*reduction_measures_2)[vertexID_new_to_old_599[v1]] = 11;
            (*reduction_measures_2)[vertexID_new_to_old_599[v2]] = 11;
            (*f_2019R1)[vertexID_new_to_old_599[v2]] = (*f_2019R1)[vertexID_new_to_old_599[v1]];
        }
        else if (condition == 2)
        {
            (*reduction_measures_2)[vertexID_new_to_old_599[v1]] = 12;
            (*reduction_measures_2)[vertexID_new_to_old_599[v2]] = 12;
            (*f_2019R1)[vertexID_new_to_old_599[v2]] = (*f_2019R1)[vertexID_new_to_old_599[v1]];
        }
    }
}

void graph_hash_of_mixed_weighted_HL_HBPLL_v1_thread_function_dij_mixed(int v_k, int N, int upper_k)
{
    ...
    while (1) // line 6
    {
        ...
        Q.push_back(vector<HBPLL_v1_node>()); // line 9
        ...
        for (int ii = 0; ii < Q_h_size; ii++) // line 11
        {
            ...
            {
                if (L_temp_599[v][i].hop + T_dij_599[used_id][L_temp_599[v][i].vertex].second <= h)
                {
                    double dis = L_temp_599[v][i].distance + T_dij_599[used_id][L_temp_599[v][i].vertex].first;
                    if (query_vk_v > dis)
                        query_vk_v = dis;
                }
            }
...
// {
//     mtx_599[max_N_ID_for_mtx_599 - 1].lock();
//     int used_id = Qid_599.front(); // which P T elements does this thread use
//     Qid_599.pop();
//     mtx_599[max_N_ID_for_mtx_599 - 1].unlock();
...
//     {
//         int L_v_k_i_vertex = L_temp_599[v_k][i].vertex;
//         T_bfs_599[used_id][L_v_k_i_vertex] = L_temp_599[v_k][i].distance; // allocate T values for L_temp_599[v_k]
//         T_changed_vertices.push(L_v_k_i_vertex);
//     }
//     mtx_599[v_k].unlock();
...
//         if (v_k <= u)
//         { // this condition is not in 2013 paper, but in 2019 paper (Lemma 3.16)
...
//             mtx_599[u].lock();
//             int L_u_size = L_temp_599[u].size();
//             for (int i = 0; i < L_u_size; i++)
//             {
//                 double dis = L_temp_599[u][i].distance +
//                              T_bfs_599[used_id][L_temp_599[u][i].vertex]; // cannot lock mtx_599[u] in this for loop,
//                                                                           // since the locking time is very large
//                 if (query_v_k_u > dis)
//                 {
//                     query_v_k_u = dis;
//                 }
...
//                 mtx_599[u].lock();
//                 L_temp_599[u].push_back(xx); // 新增标签，并行时L_temp_599[u]里面的标签不一定是按照vertex
//                     // ID排好序的，但是因为什么query时用了T_bfs_599的trick，没必要让L_temp_599[u]里面的标签排好序
//                 mtx_599[u].unlock();
//                 new_label_num++;
//                 auto u_adj_size = ideal_graph_599[u].size();
//                 for (int i = 0; i < u_adj_size; i++)
//                 {
//                     int adj_v = ideal_graph_599[u][i].first;
//                     if (P_bfs_599[used_id][adj_v] == INT_MAX)
//                     {
//                         node.vertex = adj_v;
//                         node.parent_vertex = u;
//                         node.priority_value = P_u + 1;
//                         Q.push(node);
//                         P_bfs_599[used_id][adj_v] = node.priority_value;
...
//     {
//         P_bfs_599[used_id][P_changed_vertices.front()] = INT_MAX; // reverse-allocate P values
//         P_changed_vertices.pop();
//     }
//     while (T_changed_vertices.size() > 0)
//     {
//         T_bfs_599[used_id][T_changed_vertices.front()] = INT_MAX; // reverse-allocate T values
//         T_changed_vertices.pop();
//     }

//     mtx_599[v_k].lock();
//     vector<two_hop_label_v1>(L_temp_599[v_k])
//         .swap(
//             L_temp_599[v_k]); // swap释放vector中多余空间： https://blog.csdn.net/qq_41929943/article/details/103190891
//     mtx_599[v_k].unlock();
...
//     if (labal_size_599 > max_labal_size_599)
//     {
//         throw reach_limit_error_string_MB; // after catching error, must call terminate_procedures_599(), otherwise this
...
//             .count() > max_run_time_nanoseconds_599)
//     {
//         throw reach_limit_error_string_time; // after catching error, must call terminate_procedures_599(), otherwise
//                                              // this PLL cannot be reused
//     }
// }

void graph_hash_of_mixed_weighted_HL_PLL_v1_transform_labels_to_old_vertex_IDs_element(
    vector<vector<two_hop_label_v1>> *output_L, int v_k)
{

    int L_v_k_size = L_temp_599[v_k].size();
    for (int i = 0; i < L_v_k_size; i++)
    {
        auto it = &L_temp_599[v_k][i];
        it->vertex = vertexID_new_to_old_599[it->vertex];
        it->parent_vertex = vertexID_new_to_old_599[it->parent_vertex];
    }
    sort(L_temp_599[v_k].begin(), L_temp_599[v_k].end(), compare_two_hop_label_small_to_large);

    (*output_L)[vertexID_new_to_old_599[v_k]] = L_temp_599[v_k];
    ...
    int N, int max_N_ID, int num_of_threads)
{
    ...
    {
        results.emplace_back(pool.enqueue([p, v_k] { // pass const type value j to thread; [] can be empty
...
void graph_hash_of_mixed_weighted_HBPLL_v1(graph_hash_of_mixed_weighted &input_graph, int max_N_ID, bool weighted, int num_of_threads, graph_hash_of_mixed_weighted_two_hop_case_info_v1 &case_info)
{
    ...
    /* redcution1: equivalence between vertices; we assume that vIDs in ideal_graph_599 are sorted by degree from large
     * to small*/
    if (case_info.use_2019R1)
    {
        auto begin = std::chrono::high_resolution_clock::now();
        int ideal_graph_size = ideal_graph_599.size();
        ...
        for (int i = 0; i < max_N_ID; i++)
        {
            if (case_info.f_2019R1[i] != i)
            {
                ...
                graph_v_of_v_idealID_remove_all_adjacent_edges(ideal_graph_599, vertexID_old_to_new[i]);
            }
        }
        auto end = std::chrono::high_resolution_clock::now();
        ...
        for (int x = 0; x < N; x++)
        {
            ...
            { // Prevent memory overflow
                ...
                { // Here, only one comparison is needed. A little trick.
                    case_info.reduction_measures_2019R2[vertexID_new_to_old_599[x]] = 2;
                    // cout << "reduce " << vertexID_new_to_old_599[x] << endl;
                }
            }
        }
        for (int x = N - 1; x >= 0; x--)
    ...
    int upper_k = case_info.upper_k;
    ThreadPool pool(num_of_threads);
    std::vector<std::future<int>> results;              // return typename: xxx
    int num_of_threads_per_push = num_of_threads * 100; // 每次push进去 num_of_threads_per_push线程，如果没有异常，继续push进去num_of_threads_per_push线程；如果全都一起push进去必须全部线程都结束才能catch异常
    if (weighted)
    {
        ...
        for (int i = 0; i < num_of_threads; i++)
        {
            ...
            for (int j = 0; j < N; j++)
            {
                T_dij_599[i][j].first = std::numeric_limits<double>::max();
                T_dij_599[i][j].second = 0;
            }
            Qid_599.push(i);
        }
        int push_num = 0;
        for (int v_k = 0; v_k < N; v_k++)
        {
            ...
            { // not from isolated vertices
                ...
                results.emplace_back(pool.enqueue([v_k, N, upper_k] { // pass const type value j to thread; [] can be empty
                    graph_hash_of_mixed_weighted_HL_HBPLL_v1_thread_function_dij_mixed(v_k, N, upper_k);
                    return 1; // return to results; the return type must be the same with results
                }));
    ...
    */
    ...
    for (int i = new_edges_with_middle_v.size() - 1; i >= 0; i--)
    {
        ...
        /*
            ...
            because 'parent_vertex' stands for 'next vertex on the shortest path', so it can only be shown in e1 and
           e2's labels
        */
    ...
    case_info.time_2019R2_or_enhanced_fixlabels = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin)
                                                      .count() /
                                                  1e9; // s
    ...
    if (case_info.use_canonical_repair)
    {
        ...
        if (0)
        {
            cout << "print_L_temp_599:" << endl;
            for (int i = 0; i < L_temp_599.size(); i++)
            {
                cout << "L[" << i << "]:\t";
                int ii = vertexID_old_to_new[i];
                for (int j = 0; j < L_temp_599[ii].size(); j++)
                {
                    cout << "{" << vertexID_new_to_old_599[L_temp_599[ii][j].vertex] << "," << L_temp_599[ii][j].distance << "," << vertexID_new_to_old_599[L_temp_599[ii][j].parent_vertex] << "," << L_temp_599[ii][j].hop << "}";
                }
                cout << endl;
            }
        }
    ...
    //		cout << "{" << L_temp_599[i][j].vertex << "," << L_temp_599[i][j].distance << "," <<
    // L_temp_599[i][j].parent_vertex << "}";
    //	}
    //	cout << endl;
    // }
