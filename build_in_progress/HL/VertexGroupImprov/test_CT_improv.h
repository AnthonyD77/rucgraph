...
#include <build_in_progress/HL/VertexGroupImprov/test_CT_improv.h>
...
*/
#include <build_in_progress/HL/VertexGroupImprov/CT_improv.h>
...
void test_CT_improv() {
	...
	for (int i = 0; i < iteration_graph_times; i++) {
		...
		if (1) {	
			auto begin = std::chrono::high_resolution_clock::now();
			CT_improv(instance_graph, V + 1, case_info);
			auto end = std::chrono::high_resolution_clock::now();
