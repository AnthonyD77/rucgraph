...
/*
void dgraph_PSL_v1(dgraph_v_of_v<two_hop_weight_type> &input_graph, int N, int num_of_threads,
                   dgraph_case_info_v1 &case_info)
{
    ...
    while (true)
    {
        ...
        // print_label_set(L_PSL[0]);
        // std::cout << "[Iter - In]" << std::endl;
        // print_label_set(L_PSL[1]);
        // std::cout << std::endl;
        ...
        for (int k = 0; k < 2; k++)
        {
            for (int u = 0; u < N; u++)
            {
                ...
                results.emplace_back(pool.enqueue([k, u, &G] {
                    propagate(G, k, u);
                    return 1;
                }));
            }
    ...
    for (int k = 0; k < 2; k++)
    {
        for (int i = 0; i < N; i++)
        {
            int size = L_PSL[k][i].size();
            if (size > 0)
            {
                results.emplace_back(pool.enqueue([k, i, N] { // pass const type value j to thread; [] can be empty
                    shrink(k, i, N);
    ...
    case_info.L_in = L_PSL[1];
    case_info.L_out = L_PSL[0];

    label_output_to_file("test_dgraph_label.txt", case_info.L_in, case_info.L_out);
    dgraph_clear_global_values_PSL();
}
*/

void dgraph_PSL_v2(dgraph_v_of_v<two_hop_weight_type> &input_graph, int N, int num_of_threads,
                   dgraph_case_info_v1 &case_info)
{
    std::vector<dgraph_v_of_v<two_hop_weight_type>> G(2);
    G[0] = input_graph;
    ...
    while (true)
    {
        ...
        for (int u = 0; u < N; u++)
        {
            ...
            results.emplace_back(pool.enqueue([u, &G] {
                propagate_v1(G, 0, u);
                propagate_v1(G, 1, u);
                return 1;
            }));
    ...
    //std::cout << "[Iter - Out]" << std::endl;
    //print_label_set(L_PSL_temp[0]);
    //std::cout << "[Iter - In]" << std::endl;
    //print_label_set(L_PSL_temp[1]);
    //std::cout << std::endl;

    case_info.L_in = L_PSL_temp[1];
    case_info.L_out = L_PSL_temp[0];

    label_output_to_file("test_dgraph_label.txt", case_info.L_in, case_info.L_out);
    dgraph_clear_global_values_PSL();
}

void dgraph_PSL_v3(dgraph_v_of_v<two_hop_weight_type> &input_graph, int N, int num_of_threads,
                   dgraph_case_info_v1 &case_info)
{
    std::vector<dgraph_v_of_v<two_hop_weight_type>> G(2);
    G[0] = input_graph;
    ...
    L_temp_in = case_info.L_in;
    L_temp_out = case_info.L_out;
    ...
    if (case_info.use_canonical_repair)
    {
        ...
        //cout << "before:" << case_info.label_size_before_canonical_repair << endl
             //<< "after:" << case_info.label_size_after_canonical_repair << endl;
    }
