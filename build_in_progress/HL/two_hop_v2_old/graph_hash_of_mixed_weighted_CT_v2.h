...
#include <build_in_progress/HL/two_hop_v1_old/graph_hash_of_mixed_weighted_PLL_v1.h>
#include <build_in_progress/HL/two_hop_v1_old/graph_hash_of_mixed_weighted_PSL_v1.h>
#include <build_in_progress/HL/two_hop_v1_old/graph_hash_of_mixed_weighted_PLL_dummy_v1.h>
#include <build_in_progress/HL/two_hop_v1_old/VCPLL.h>
...
public:
    ...
    int d = 3;
    // set d as a large number to test the correctness of CT-index
    ...
    void record_all_details(string save_name) {
        ...
        outputFile << "d=" << d << endl;
        outputFile << "use_P2H=" << use_P2H << endl;
        ...
        outputFile << "time_P2H=" << time_P2H << endl;
        outputFile << "time_extension=" << time_extension << endl;
...
vector<vector<midnode>> global_midnode_graph;
...
void CT_v2(graph_hash_of_mixed_weighted& input_graph, int max_N_ID, graph_hash_of_mixed_weighted_CT_v2_case_info& case_info) {
    ...
    */
    ...
    if (case_info.use_P2H) {
        ...
        std::vector<std::vector<pair<int, double>>> ZBags(N);  // bag nodes of decomposied tree
    ...
    case_info.time_P2H = std::chrono::duration_cast<std::chrono::nanoseconds>(end_p2h - begin_p2h).count() / 1e9;
    //---------------------------------------------------P2H end---------------------------------------------------
...
double CT_extract_distance(graph_hash_of_mixed_weighted_CT_v2_case_info& case_info, int source, int terminal) {
    ...
    if (!isIntree[source] && !isIntree[terminal])  // both on core, use PLL indexs only
    {
        ...
        return graph_hash_of_mixed_weighted_two_hop_v1_extract_distance(L, case_info.two_hop_case_info.reduction_measures_2019R2, case_info.two_hop_case_info.reduction_measures_2019R1, case_info.two_hop_case_info.f_2019R1, case_info.core_graph, source, terminal);
    }
    ...
    else if (root[source] != root[terminal])  // locate in different trees, have not used lamma 9 here yet
    {
        ...
        for (int i = 0; i < r_s_size; i++) {
            ...
            for (int j = 0; j < r_t_size; j++) {
                ...
                double d_u_w = graph_hash_of_mixed_weighted_two_hop_v1_extract_distance(L, case_info.two_hop_case_info.reduction_measures_2019R2, case_info.two_hop_case_info.reduction_measures_2019R1, case_info.two_hop_case_info.f_2019R1, case_info.core_graph, u, w);
