...
void PI11(graph_hash_of_mixed_weighted& instance_graph, vector<vector<two_hop_label_v1>>& L,
	...
	for (auto it = al1_curr.begin(); it != al1_curr.end(); it++) {
		...
		insert_sorted_two_hop_label(L[it->first], it->second, std::numeric_limits<weightTYPE>::max());
	}
}
...
void PI12(graph_hash_of_mixed_weighted& instance_graph, vector<vector<two_hop_label_v1>>& L, PPR_type& PPR,
	...
	for (auto it = al1_curr.begin(); it != al1_curr.end(); it++) {
		std::vector<int> temp = PPR_retrieve(PPR, it->first, it->second);
		...
		for (auto t = temp.begin(); t != temp.end(); t++) {
			if (it->first < *t) { //  && !nop.count(pair_label(*t, it->first))
				...
				for (auto nei = neis.begin(); nei != neis.end(); nei++) {
					d1 = min(d1, search_sorted_two_hop_label(L[nei->first], it->first) + (weightTYPE)nei->second);
				}
				auto query_result = Query2(*t, it->first);
				if (query_result.first > d1) {
					insert_sorted_two_hop_label(L[*t], it->first, d1);
					al2_next.push_back(pair_label(*t, it->first));
					//nop.insert(pair_label(*t, it->first));
					//PPR_erase(mm.PPR, it->first, it->second, *t); // update PPR
				}
				else {
					PPR_insert(PPR, *t, query_result.second, it->first);
					PPR_insert(PPR, it->first, query_result.second, *t);
				}
			}
			if (*t < it->first) { //  && !nop.count(pair_label(it->first, *t))
				...
				auto query_result = Query2(it->first, *t);
				if (query_result.first > d1) {
					insert_sorted_two_hop_label(L[it->first], *t, d1);
	...
	for (auto it = al2_curr.begin(); it != al2_curr.end(); it++) {
		...
		for (auto nei = v_neis.begin(); nei != v_neis.end(); nei++) {
			...
			if (nei->first > it->second) {
				...
				auto query_result = Query2(nei->first, it->second);
...
void WeightIncreaseMaintenance(graph_hash_of_mixed_weighted& instance_graph, graph_hash_of_mixed_weighted_two_hop_case_info_v1& mm, int v1, int v2, weightTYPE w_old, weightTYPE w_new) {
	...
	while (al1_curr.size() || al2_curr.size()) {
		PI11(instance_graph, mm.L, al1_curr, al1_next, w_new - w_old);
		PI12(instance_graph, mm.L, mm.PPR, al1_curr, al2_next); //, nop);
		PI22(instance_graph, mm.L, mm.PPR, al2_curr, al2_next); //, nop);
		al1_curr = al1_next;
		al2_curr = al2_next;
