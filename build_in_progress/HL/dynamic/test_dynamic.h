...
void graph_change_and_label_maintenance(graph_hash_of_mixed_weighted& instance_graph, graph_hash_of_mixed_weighted_two_hop_case_info_v1& mm,
	...
	while (weightIncrease_time + weightDecrease_time) {
		...
		if (weightIncrease_time >= weightDecrease_time) {
			...
			WeightIncreaseMaintenance_improv(instance_graph, mm, selected_edge.first, selected_edge.second, selected_edge_weight, new_ec);
			...
			/*maintain labels*/
			WeightDecreaseMaintenance_improv(instance_graph, mm, selected_edge.first, selected_edge.second, new_ec, thread_num);
	...
	for (int i = 0; i < iteration_graph_times; i++) {
		...
		if (use_WeightIncreaseMaintenance) {
			...
			for (int j = 0; j < weightChange_time; j++) {
				double new_ec = min(graph_hash_of_mixed_weighted_edge_weight(g, selected_edges[j].first, selected_edges[j].second) * (1 + weightChange_ratio), 1e6);
				graph_hash_of_mixed_weighted_add_edge(g, selected_edges[j].first, selected_edges[j].second, new_ec);
				auto begin = std::chrono::high_resolution_clock::now();
				WeightIncreaseMaintenance(g, mm2, selected_edges[j].first, selected_edges[j].second, graph_hash_of_mixed_weighted_edge_weight(g, selected_edges[j].first, selected_edges[j].second), new_ec);
				auto end = std::chrono::high_resolution_clock::now();
				double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
		...
		if (use_WeightIncreaseMaintenance_improv) {
			...
			for (int j = 0; j < weightChange_time; j++) {
				...
				graph_hash_of_mixed_weighted_add_edge(g, selected_edges[j].first, selected_edges[j].second, new_ec);
				auto begin = std::chrono::high_resolution_clock::now();
				WeightIncreaseMaintenance_improv(g, mm2, selected_edges[j].first, selected_edges[j].second, graph_hash_of_mixed_weighted_edge_weight(g, selected_edges[j].first, selected_edges[j].second), new_ec);
				auto end = std::chrono::high_resolution_clock::now();
				double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
		...
		if (use_WeightDecreaseMaintenance) {
			...
			for (int j = 0; j < weightChange_time; j++) {
				...
				graph_hash_of_mixed_weighted_add_edge(g, selected_edges[j].first, selected_edges[j].second, new_ec);
				auto begin = std::chrono::high_resolution_clock::now();
				WeightDecreaseMaintenance(g, mm2, selected_edges[j].first, selected_edges[j].second, graph_hash_of_mixed_weighted_edge_weight(g, selected_edges[j].first, selected_edges[j].second), new_ec);
				auto end = std::chrono::high_resolution_clock::now();
				double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
		...
		if (use_WeightDecreaseMaintenance_improv) {
			...
			for (int j = 0; j < weightChange_time; j++) {
				...
				graph_hash_of_mixed_weighted_add_edge(g, selected_edges[j].first, selected_edges[j].second, new_ec);
				auto begin = std::chrono::high_resolution_clock::now();
				WeightDecreaseMaintenance_improv(g, mm2, selected_edges[j].first, selected_edges[j].second, new_ec, thread_num);
				auto end = std::chrono::high_resolution_clock::now();
				double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
