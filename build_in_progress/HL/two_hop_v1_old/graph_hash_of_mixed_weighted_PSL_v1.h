...
void graph_hash_of_mixed_weighted_PSL_v1_thread_function_dij(int u, graph_hash_of_mixed_weighted_two_hop_case_info_v1* case_info)
{
	...
	/* save labels of u in T */ //�������T֮��,�����query�ͻ��һ��,���Ӧ�ò��ø�
	...
	/*
	if (reduction_measures[vertexID_new_to_old[u]] == 2) //���u��lms�еĵ�,��ô�൱������ɾ������,����Ҫ�ٸ�����label��
	{
		for (int i = 0; i < u_label_size; i++)
	...
	*/
	...
	int u_adj_size = ideal_graph_595[u].size(); // u���ھӵ�����
	for (int i = 0; i < u_adj_size; i++)        // ����u���ھ�v
	{
		int v = ideal_graph_595[u][i].first;
		double ec = ideal_graph_595[u][i].second;        //(u,v)����
		int v_label_size = pos_595[v] + increment_595[v]; // increment_595[v] is label size in the last iteration
		if (reduction_measures_2019R2[vertexID_new_to_old_595[v]] == 2) //�����v��lms�����еĵ�,��ô�������ѭ��,�ҵ�v����u������ڽӵ�(N2����)
		{		
			int v_adj_size = ideal_graph_595[v].size();
			for (int j = 0; j < v_adj_size; j++)
			{
				if (ideal_graph_595[v][j].first == u) //��Ҫ�ų���u������
					continue;
				int v_1 = ideal_graph_595[v][j].first;     //���ǵ�v1��index
				double ec_2 = ideal_graph_595[v][j].second; // v��v1�ıߵĳ���
				//����v_1��d-2����label,����pos_2
		...
		else // v����lms�����еĵ�,��ô�������ѭ��,�ҵ�v��d-1label(N1����)
		{
...
void graph_hash_of_mixed_weighted_PSL_v1_thread_function_add_new_labels(int u)
{
	...
	vector<two_hop_label_v1>(L_595[u]).swap(L_595[u]);  // swap�ͷ�vector�ж���ռ䣺 https://blog.csdn.net/qq_41929943/article/details/103190891 
}
...
(graph_hash_of_mixed_weighted& input_graph, int max_N_ID, int num_of_threads, graph_hash_of_mixed_weighted_two_hop_case_info_v1& case_info) {
	...
	int num_of_threads_per_push = num_of_threads * 1000; // ÿ��push��ȥ num_of_threads_per_push �̣߳����û���쳣������push��ȥnum_of_threads_per_push�̣߳����ȫ��һ��push��ȥ����ȫ���̶߳���������catch�쳣
